...
namespace MattDavinoProject.Data
{
    public class AuthRepository : IAuthRepository
    {
        ...
        public async Task<User> Register(string userName, string password, string gender, DateTime birthday, string name, string city)
        {
            // Hash the password using SHA512 with random key (salt)
            var hash = new HMACSHA512();
            var computedHash = hash.ComputeHash(System.Text.Encoding.UTF8.GetBytes(password));

            var newUser = new User { UserName = userName, Gender=gender, BirthDate=birthday, Name=name, City=city};
